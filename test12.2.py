...
def quat_from_euler(roll, pitch, yaw):
    ...
    Returns an instance of math_helpers.Quat.
    """
    cy = math.cos(yaw * 0.5)
    sy = math.sin(yaw * 0.5)
...
def adjust_spot_posture(height=0.1, pitch=0.2):
    ...
    # Use the custom Euler-to-quaternion conversion.
    # Experiment with the pitch sign/magnitude to get the desired front lift.
        ...
        if len(det) >= 6:
            ...
            if int(class_id) == 0:  # Class ID 0 is for "person"
                ...
                if distance > target_distance + distance_threshold:
                    ...
                    adjust_spot_posture(height=0.0, pitch=0.0)  # Revert to default posture
                    ...
                    adjust_spot_posture(height=0.1, pitch=0.2)  # Lift front to improve visibility
                else:
